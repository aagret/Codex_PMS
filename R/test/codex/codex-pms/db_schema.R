...
get_constraint_columns <- function(con, schema, table, constraint_name) {
    sql <- "
    ...
    DBI::dbGetQuery(con, sql, params = list(schema, table, constraint_name))$column_name
}
...
ensure_table_from_dt <- function(con, DT, schema, table, keys,
    ...
    uq_exists <- constraint_exists(con, schema, table, unique_name)
    if (uq_exists) {
        existing_uq_cols <- get_constraint_columns(con, schema, table, unique_name)
        if (!identical(existing_uq_cols, keys)) {
            DBI::dbExecute(con, paste0(
                "ALTER TABLE ", q_full_table,
                " DROP CONSTRAINT ", quote_ident(con, unique_name), ";"
            ))
            uq_exists <- FALSE
        }
    }
